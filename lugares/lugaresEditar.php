...
<?php 
    ...
    //obtengo el lugar creacion
        $id = $_POST['id'];
        $curs = oci_new_cursor($conn);

        // Preparación de la consulta que llama al procedimiento almacenado
        $query = "BEGIN obtener_lugar_creacion(:param1, :id); END;";
        $stmt = oci_parse($conn, $query);
...
<div class="container">
    <form method='post' action='editar.php'>
        ...
        <div class="form-group">
            ...
            <input type="text" value="<?php echo $lugar_creacion['DIRECCION'] ?>" name="direccion" class="form-control" id="direcicon" placeholder="Conpcecion de Alajuelita" disabled>
        </div>

        <div class="form-group">
            <label for="exampleInputEmail1">Digite el canton</label>
            <input type="text" value="<?php echo $lugar_creacion['CANTON'] ?>" name="canton" class="form-control" id="canton" placeholder="Alajuelita" disabled>
        </div>

        <input type="text" name="id"  value="<?php echo $lugar_creacion['ID']?>" hidden>
